...
class Lingva extends \Opencart\System\Engine\Controller {
  ...
  private function _translate(string $string) {

    $string = urlencode(str_replace('/', '|', strip_tags(preg_replace('#<br\s*/?>#i', "\n", $string))));

    $values = [];
    foreach ((array) explode('%0A', $string) as $value) {

      if (!empty(trim(preg_replace('/\t+/', '', urldecode($value))))) {
    ...
    return nl2br(str_replace('|', '/', implode("\n", $values)));
  }
}
